Compiler = function () {
    ...
    this.run = function () {
        ...
        var keepGoing = (currentInstr < compiledCode.length);
        var i = 10000;
        ...
        if (keepGoing && !stop) {
            var self = this, to = setTimeout(function () {
                to = null;
                self.run()
            }, 0);
        } else {
    ...
    this.getErrors = function (code) {
        var errors = [], loop = {opened: 0, closed: 0};

        for (var i = 0; i < code.length; i++) {
            if (authorizedChars.indexOf(code.charAt(i)) === -1) {
                var error = {};
                error.type = "Unauthorized instruction";
                error.addr = i;
        ...
        if (loop.opened != loop.closed) {
            var error = {};
            error.type = (loop.opened > loop.closed) ? (loop.opened - loop.closed) + " \"]\" missing" : (loop.closed - loop.opened) + " \"[\" missing";
            error.addr = NaN;
            error.char = NaN;
            errors.push(error);
        }
